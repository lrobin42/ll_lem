...
pub fn create_token_text_array(text: &String) -> Vec<String> {
    //regex split text into vector of tokens and characters
    let re = Regex::new(r#"([,.:;?_!"()\']|--|\s)"#).unwrap();

    // Split by regex and collect into Vec<String>
    let token_text: Vec<String> = re.split(text).map(|s| s.to_string()).collect();
    token_text
}

//create input and target vectors x and y
pub fn create_x_y_vectors(token_vec: &Vec<u32>) -> (Vec<Vec<u32>>, Vec<Vec<u32>>) {
    ...
    let mut i = 0;
    while i + context_size < token_vec.len() {
        let input_chunk = token_vec[i..i + context_size].to_vec();
        let target_chunk = token_vec[i + 1..i + context_size + 1].to_vec();
...
pub fn create_embeddings(token_vec: &Vec<u32>) -> Tensor<NdArray, 3> {
    ...
    let tokens_i32: Vec<i32> = token_vec.iter().map(|&t| t as i32).collect();
...
pub fn add_positional_embeddings() {
